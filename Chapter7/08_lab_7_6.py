...
max_population = 0 #변수 만들기
min_population = 99999999999999
total_population = 0

max_city = None #변수 비워두기
min_city = None

for city in city_info: #city가 city_info 안에 있을 경우 반복실행
    total_population += city[1] #도시 개수 세기
    if city[1] > max_population: #만약 city가 max보다 클 경우
        max_population= city[1] #max 변수를 변경
        max_city = city #max 변수를 city로 변경
    if city[1] < min_population: #만약 city가 min보다 작을 경우
        min_population = city[1] #min 변수를 더 작은 것으로 변경
